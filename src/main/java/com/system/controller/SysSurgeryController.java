...
public class SysSurgeryController {
    ...
    public List<String> getList(@PathVariable int hArea) {
        return sysSurgeryService.getNameList(hArea);
    }

    @ApiOperation(value = "按照名字获取相应的列表")
    @RequestMapping(value = "/getListByName/", method = RequestMethod.POST)
    @ResponseStatus(HttpStatus.OK)
    public List<SysSurgeryDTO> getListByName(@RequestBody List<String> names) {
        return sysSurgeryService.getListByName(names);
    }
}
